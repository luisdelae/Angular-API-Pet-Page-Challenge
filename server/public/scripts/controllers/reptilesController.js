myApp.controller('ReptilesController', ['$scope', '$http', function($scope, $http) {
  $scope.data = {};
  ...
  function petFinder() {
    ...
    $http.jsonp(request1).then(
      function(response) {
        ...
        var shelterId;
        console.log(resObj);
        $scope.reptilepicture = resObj.media.photos.photo[2].$t;
        $scope.reptilename = resObj.name.$t;
        $scope.reptileage = resObj.age.$t;
        $scope.reptilebreed = resObj.breeds.breed.$t;
        $scope.reptilesex = resObj.sex.$t;
        $scope.reptiledescription = resObj.description.$t;
